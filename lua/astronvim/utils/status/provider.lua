...
function M.filename(opts)
  ...
  return function(self)
    ...
    if opts.modify == "smart" then
      -- Ensure that the path is at least one level above HOME
      if condition.is_in_project() then
        filename = status_utils.get_relative_path(path)
      else
    ...
    if opts.post_modify then filename = opts.post_modify(filename) end
...
function M.unique_path(opts)
  opts = extend_tbl({
    buf_name = function(bufnr)
      ...
      return success and vim.fn.fnamemodify(name, ":t") or ""
    end,
    bufnr = 0,
    max_length = 16,
  }, opts)
...
---@param opts? table options passed to the stylize function
---@return function # the function for outputting the cwd
-- @usage local heirline_component = { provider = require("astronvim.utils.status").provider.cwd() }
-- @see astronvim.utils.status.utils.stylize
function M.cwd(opts)
  opts = extend_tbl({ --[[ truncate = 0.25 ]] }, opts)
  return function()
    local cwd = vim.fn.fnamemodify(vim.fn.getcwd(), ":~")
    if type(opts.truncate) == "number" then
      local max_width = math.floor(status_utils.width() * opts.truncate)
      if #cwd > max_width then cwd = string.sub(cwd, 0, max_width) .. "…" end
    end
    return status_utils.stylize(cwd, opts)
  end
end
