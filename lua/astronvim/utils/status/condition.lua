...
--  A condition function if the current buffer is in CWD
---@return boolean # true if the current buffer is in CWD, false otherwise
function M.is_in_cwd()
  local cwd = vim.fn.getcwd()
  local buf_abs_path_str = vim.fn.expand "%:p"
  return vim.startswith(buf_abs_path_str, cwd)
end

-- A condition function if the current buffer shares the same first folder from home ~/<folder>/
function M.is_in_project(path)
  -- Compare if the first directory after home is the same
  -- Ensure that the path is at least one level above HOME
  path = path or vim.fn.expand "%:p"
  local home = vim.fn.expand "$HOME"
  local function getFirstFolder(_path) return _path:match "^~/(.-)/" end

  -- Extracting the first folder from both paths
  return getFirstFolder(home) == getFirstFolder(path)
end
return M
