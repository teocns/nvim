...
function M.nav(opts)
  ...
  return M.builder(status_utils.setup_providers(opts, { "ruler", "percentage", "command", "scrollbar" }))
end
...
--- Shows us the current project-nvim CWD
---@param opts? table a list of components to build into a section
---@return table # The Heirline component table
-- @usage
function M.project(opts)
  ...
  opts = extend_tbl({
    cwd = {
      hl = hl.get_attributes("buffer_overflow", false),
      condition = function() return condition.is_in_project() end,
    },
    filename = {
      icon = { kind = "Selected", padding = { left = 1, right = 1 } },
      -- Relative path
      modify = "smart",
      -- post_modify = function(path) return " > " .. path end,
      hl = hl.get_attributes("buffer_active", false),
    },
  }, opts)
  -- We need to use a custom `setup` for `setup_providers` because of duplicates
  return M.builder(status_utils.setup_providers(opts, { "cwd", "filename" }))
end
