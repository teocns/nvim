...
local conditions = require "astronvim.utils.status.condition"
...
function M.get_relative_path(filepath)
    -- Normalizing the paths by removing trailing slashes
    local directory = vim.fn.getcwd()
    directory = directory:gsub("[/\\]+$", "")
    filepath = filepath:gsub("[/\\]+$", "")

    -- Splitting the paths into parts
    local dir_parts = {}
    ...
    local i = 1
    while i <= #dir_parts and i <= #file_parts and dir_parts[i] == file_parts[i] do
        i = i + 1
    end

    -- Construct the relative path
    local relative_parts = {}
    for j = i, #dir_parts do
