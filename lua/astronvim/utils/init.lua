...
function M.toggle_term_cmd(opts)
  ...
  -- if not opts.cmd then opts.cmd = vim.o.shell end
...
-- ---@param opts table A table of options for Terminal:new() (Check toggleterm.nvim documentation for table format)
...
--   local key = opts.cmd or "default"
--   -- if terminal doesn't exist yet, create it
--   if not terms[key] then terms[key] = {} end
--   if not terms[key][num] then
--     if not opts.count then opts.count = vim.tbl_count(terms) * 100 + num end
--     if not opts.on_exit then opts.on_exit = function() terms[key][num] = nil end end
--     terms[key][num] = require("toggleterm.terminal").Terminal:new(opts)
--   else
--     -- update the direction of the existing terminal
--     terms[key][num].direction = opts.direction
--   end
--   -- toggle the terminal
