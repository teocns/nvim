...
function M.close_all(keep_current, force)
  ...
  for _, bufnr in ipairs(vim.t.bufs) do
    if not keep_current or not visible_buffers[bufnr] then M.close(bufnr, force) end
  end
end
...
-- function M.filename()
--   -- Gets current or last known valid filename
--   -- Try get filename of current buffer, else get last known buffer filename
--   pcall(function() M.current_buf = vim.api.nvim_get_current_buf() end)
--   if M.current_buf and M.is_valid(M.current_buf) then
--     return vim.api.nvim_buf_get_name(M.current_buf)
--   end
--   pcall(function() M.last_buf = vim.api.nvim_get_current_buf() end)
--   if M.last_buf and M.is_valid(M.last_buf) then
--     return vim.api.nvim_buf_get_name(M.last_buf)
--   end
--   return "No buffer?"
-- end
